...
The `licenseseat:default` permission grants only the normal end-user lifecycle and read-only release commands. Explicitly opt in to admin, arbitrary-key, offline-artifact, download-token, or reset commands only when the corresponding webview needs them. See [Permissions](#permissions).
...
| `maxOfflineDays` | `number` | `0` | Grace period in days; `0` disables offline authority |
...
|-------|---------|-------------|
| `licenseseat://activation-success` | `{ activatedAt }` | License activated |
| `licenseseat://activation-error` | `string` | Activation failed |
| `LICENSESEAT_EVENTS.VALIDATION_SUCCESS` | `{ valid, code, offline }` | Validation succeeded |
| `LICENSESEAT_EVENTS.VALIDATION_FAILED` | `{ valid, code, offline }` | Validation failed |
| `LICENSESEAT_EVENTS.LICENSE_LOADED` | `{ activatedAt }` | Cached license loaded on startup |
| `LICENSESEAT_EVENTS.LICENSE_REVOKED` | `string \| null` | License was revoked |
...
Event payloads are deliberately minimal because Tauri events are app-global. They never include license keys, fingerprints, activation IDs, IP addresses, certificates, or metadata. Use permission-scoped commands such as `getState()` for full state, `subscribeState()` for refreshed snapshots, and `listenEvent()` when you only need lifecycle notification.
...
| Mode | Description |
|------|-------------|
| `network_only` | Fall back only for network/timeout/5xx failures; never for business-rule failures (default) |
| `always` | Try cached machine files after any non-business failure, then legacy tokens if explicitly enabled |

Set `maxOfflineDays` to a positive value to enable offline authority. A value of `0` prevents
automatic artifact fetching, refresh, and cached-artifact authorization regardless of mode.
...
|------------|-------------|
| `licenseseat:default` | Least-privilege end-user lifecycle and read-only release commands (recommended) |
...
The default set intentionally excludes `get-admin-snapshot`, explicit arbitrary-key operations, raw offline artifact/key helpers, download-token generation, and reset. Grant those command permissions to the narrowest capability/window that needs them; never grant admin helpers to remote-content windows.
